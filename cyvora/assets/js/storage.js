 ...
 *
 * Usage:
 *   CyvoraStore.set("settings", { lang: "fr", ... });   // JSON-serialized
 *   CyvoraStore.get("settings", {});                      // parsed, with default
 *   CyvoraStore.remove("settings");
 *   CyvoraStore.setToken(tok) / getToken() / clearAuth();
 *   CyvoraStore.getPreferences() / setPreferences(obj);   // AI/user prefs
 *
...
(function () {
  ...
  var NS = "cyvora:"; // namespace so keys are easy to find/clear

  function cfg() { return window.CyvoraConfig || {}; }
  ...
  function clearAll() {
    ...
    try {
      ...
      for (var i = 0; i < ls.length; i++) {
        var k = ls.key(i);
        if (k && (k.indexOf(NS) === 0 || k.indexOf("cyvora_") === 0)) toRemove.push(k);
      }
  ...
  function getToken() {
    ...
    try { return ls.getItem(cfg().AUTH_TOKEN_KEY || "cyvora_token"); } catch (e) { return null; }
  }
  function setToken(tok) {
    var ls = raw(); if (!ls) return;
    try { ls.setItem(cfg().AUTH_TOKEN_KEY || "cyvora_token", tok); } catch (e) { /* ignore */ }
  }
  function getRefreshToken() {
    var ls = raw(); if (!ls) return null;
    try { return ls.getItem(cfg().REFRESH_TOKEN_KEY || "cyvora_refresh"); } catch (e) { return null; }
  }
  function setRefreshToken(tok) {
    var ls = raw(); if (!ls || !tok) return;
    try { ls.setItem(cfg().REFRESH_TOKEN_KEY || "cyvora_refresh", tok); } catch (e) { /* ignore */ }
  }
  function getUser() {
    var ls = raw(); if (!ls) return null;
    try { return JSON.parse(ls.getItem(cfg().USER_KEY || "cyvora_user") || "null"); } catch (e) { return null; }
  }
  function setUser(user) {
    var ls = raw(); if (!ls) return;
    try { ls.setItem(cfg().USER_KEY || "cyvora_user", JSON.stringify(user || null)); } catch (e) { /* ignore */ }
  }
  function clearAuth() {
    var ls = raw(); if (!ls) return;
    try {
      ls.removeItem(cfg().AUTH_TOKEN_KEY || "cyvora_token");
      ls.removeItem(cfg().REFRESH_TOKEN_KEY || "cyvora_refresh");
      ls.removeItem(cfg().USER_KEY || "cyvora_user");
    } catch (e) { /* ignore */ }
  }

  // ---- User preferences (AI personalization + settings, SRS FR-05/FR-37) --
  // Cached locally as JSON; the source of truth is the backend
  // (GET/PUT /ai/preferences). CyvoraAPI.preferences syncs these.
