 ...
 * sessionStorage under "cyvora_post_login_redirect" so login.html can
...
(function () {
  ...
  var key = (window.CyvoraConfig && window.CyvoraConfig.AUTH_TOKEN_KEY) || "cyvora_token";
    ...
    if (location.protocol === "file:") {
      window.addEventListener("DOMContentLoaded", function () {
        ...
        bar.textContent =
          ...
          "Run  python3 -m http.server  in the cyvora folder and open http://localhost:8000/public/ instead.";
  ...
  if (token) {
    // Admin role gate (scaffold). Flip ENFORCE_ADMIN_ROLE on once the real
    // backend returns roles; off for now so demo admin screens stay reachable.
    var ENFORCE_ADMIN_ROLE = false;
    if (ENFORCE_ADMIN_ROLE && location.pathname.toLowerCase().indexOf("/admin/") !== -1) {
      var role = null;
      try {
        var u = JSON.parse(window.localStorage.getItem("cyvora_user") || "null");
        role = u && u.role;
      } catch (e) { /* ignore */ }
      if (role !== "admin") {
  ...
  try {
    sessionStorage.setItem("cyvora_post_login_redirect", location.pathname + location.search);
  } catch (e) { /* ignore */ }
