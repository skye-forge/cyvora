...
class DashboardView(APIView):
    ...
    def get(self, request):
        ...
        recent_incidents = Incident.objects.filter(reporter=user).order_by("-submitted_at")[:3]
        recent_lessons = LessonProgress.objects.filter(user=user).order_by("-completed_at")[:3]
        ...
        return success_response(data={
            ...
            "summary": {
                ...
                "pending_reports": Incident.objects.filter(reporter=user, status=Incident.STATUS_PENDING).count(),
                "resolved_reports": Incident.objects.filter(reporter=user, status__in=[Incident.STATUS_APPROVED, Incident.STATUS_REJECTED]).count(),
                "lessons_completed": LessonProgress.objects.filter(user=user, completed=True).count(),
                "rank": ranking["rank"],
                "percentile": ranking["percentile"],
            },
            "recent_activity": {
                "incidents": IncidentListSerializer(recent_incidents, many=True).data,
