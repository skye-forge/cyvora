...
(function () {
  ...
  async function tryRefresh() {
    ...
    try {
      ...
      if (data && data.token && window.VarnisStore) {
        window.VarnisStore.setToken(data.token);
        if (data.refreshToken) window.VarnisStore.setRefreshToken(data.refreshToken);
        return true;
      }
  ...
  function persistAuth(res) {
    if (res && window.VarnisStore) {
      if (res.token) window.VarnisStore.setToken(res.token);
      if (res.refreshToken) window.VarnisStore.setRefreshToken(res.refreshToken);
