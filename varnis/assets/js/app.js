/**
 * Varnis — page bootstrap & data loaders
 * ------------------------------------------------------------
 * Detects which page is open and wires VarnisAPI + templates.
 * All data is JSON from the API layer (mock or live).
 */
(function () {
  "use strict";

  var T = window.VarnisTemplates;
  var API = window.VarnisAPI;
  ...
  function showToast(msg, type) {
    ...
    console.log("[Varnis]", type || "info", msg);
  }
  ...
  function bootPublicIndex() {
    ...
    if (alertsHost) {
      if (window.VarnisUI) window.VarnisUI.showLoading(alertsHost, 'skeleton-card');
  ...
  function bootMyReports() {
    ...
    if (catSel) {
      var cats = (window.VarnisMockData && window.VarnisMockData.reportCategories) || [];
    ...
    if (window.VarnisUI) window.VarnisUI.showLoading(tbody, 'skeleton-table');
    (API.reports.mine ? API.reports.mine() : API.reports.list()).then(function (res) {
      all = res.items || [];
      fillTiles();
      render(all);
    }).catch(function () {
  ...
  function bootPublicCommunity() {
    ...
    if (window.VarnisUI) window.VarnisUI.showLoading(host, 'skeleton-table');
    ...
    if (form) {
      ...
      form.addEventListener("submit", function (e) {
        ...
        if (window.VarnisValidation) {
          var v = window.VarnisValidation.validateCommunityPost(postPayload);
          if (!v.valid) {
            window.VarnisValidation.showValidationErrors(v.errors);
            return;
          }
        ...
        API.community.create(postPayload)
          ...
          .catch(function (err) {
            if (err && err.validationErrors) {
              window.VarnisValidation ? window.VarnisValidation.showValidationErrors(err.validationErrors) : showToast(err.message, "error");
            } else {
  ...
  function bootLearn() {
    ...
    if (window.VarnisUI) window.VarnisUI.showLoading(grid, 'skeleton-card');
    API.learning.courses().then(function (res) {
      var items = res.items || [];
  ...
  function bootReportIncident() {
    ...
    function populateReviewSummary() {
      ...
      var escFn = (window.VarnisTemplates && window.VarnisTemplates.esc) || function (s) {
    ...
    // --- Auto-save Draft ---
    var DRAFT_KEY = 'varnis_report_draft';
    ...
    function doSubmit() {
      ...
      if (window.VarnisValidation) {
        var v = window.VarnisValidation.validateReport(reportPayload);
        if (!v.valid) {
          window.VarnisValidation.showValidationErrors(v.errors);
          return;
        }
      ...
      API.users.me().then(function (user) {
        ...
        if (err && err.validationErrors && window.VarnisValidation) {
          window.VarnisValidation.showValidationErrors(err.validationErrors);
        } else {
  ...
  function bootProfile() {
    ...
    var savedPhoto = window.VarnisStore ? window.VarnisStore.get("profile_photo", null) : null;
    ...
    if (photoInput) {
      photoInput.addEventListener("change", function () {
        ...
        reader.onload = function () {
          ...
          if (window.VarnisStore) window.VarnisStore.set("profile_photo", reader.result);
  ...
  function bootAdminReports() {
    ...
    if (!tbody) return;
    if (window.__varnisAdminReportsBound) return;
  ...
  function bootAdminAlerts() {
    ...
    form.addEventListener("submit", function (e) {
      ...
      if (window.VarnisValidation) {
        var v = window.VarnisValidation.validateAlert(alertPayload);
        if (!v.valid) {
          window.VarnisValidation.showValidationErrors(v.errors);
          return;
        }
  ...
  function bootSettings() {
    ...
    form.addEventListener("submit", function (e) {
      ...
      if ((f.sub_weekly || f.sub_monthly || f.sub_quarterly) && f.sub_email && window.VarnisValidation) {
        var v = window.VarnisValidation.validateEmail(f.sub_email);
  ...
  function bootNotifications() {
    var list = $("[data-notifications-list]");
    if (!list) return;

    var empty = $("[data-notif-empty]");
    var summary = $("[data-notif-summary]");
    var markAllBtn = $("[data-mark-all-read]");
    var filterBar = $("[data-notif-filters]");
    var all = [];
    var activeFilter = "all";

    function t(key, fallback) {
      return (window.VarnisI18n && window.VarnisI18n.t && window.VarnisI18n.t(key)) || fallback;
    }
    ...
    function updateChrome() {
      ...
      if (summary) {
        summary.textContent = unread > 0
          ? unread + " " + (unread === 1 ? t("notif.one_unread", "unread update") : t("notif.many_unread", "unread updates")) + " · " + all.length + " " + t("notif.total", "total")
          : t("notif.none_unread", "You're all caught up — no unread updates.");
      }
      if (markAllBtn) markAllBtn.disabled = unread === 0;
      // Chip counters
      var cAll = $('[data-count="all"]', filterBar);
      var cUnread = $('[data-count="unread"]', filterBar);
      if (cAll) cAll.textContent = all.length ? "· " + all.length : "";
      if (cUnread) cUnread.textContent = unread ? "· " + unread : "";
      // Refresh the topbar bell badge to match
      if (window.VarnisUpdateBellBadge) window.VarnisUpdateBellBadge(unread);
    }

    function visibleItems() {
      if (activeFilter === "all") return all;
      if (activeFilter === "unread") return all.filter(function (n) { return !n.read; });
      return all.filter(function (n) { return n.type === activeFilter; });
    }
    ...
    function render() {
      ...
      if (!items.length) {
        ...
        if (empty) {
          ...
          // Contextual empty copy when a filter is active but data exists
          var title = $("h3", empty);
          if (title) {
            title.textContent = all.length && activeFilter !== "all"
              ? t("notif.empty_filtered", "Nothing here for this filter")
              : t("notif.empty_title", "You're all caught up");
          }
        }
      } else {
        if (empty) empty.classList.add("hidden");
        list.innerHTML = items.map(T.notificationItem).join("");
        // Rows are injected after site.js's reveal observer ran — flip them
        // in-view on the next frame so the entrance transition still plays.
        requestAnimationFrame(function () {
          $all(".reveal", list).forEach(function (el) { el.classList.add("in-view"); });
        });
      }
      updateChrome();
    }
    ...
    list.addEventListener("click", function (e) {
      ...
      if (el) openNotification(el);
    });
    list.addEventListener("keydown", function (e) {
      if (e.key !== "Enter" && e.key !== " ") return;
      var el = e.target.closest("[data-notif-id]");
      if (el) { e.preventDefault(); openNotification(el); }
    });
    ...
    if (markAllBtn) {
      markAllBtn.addEventListener("click", function () {
        ...
        API.notifications.markAllRead().then(function () {
          showToast(t("notif.all_read_toast", "All notifications marked as read."), "success");
        }, function () {
          showToast(t("notif.all_read_fail", "Could not sync with the server — will retry next visit."), "error");
        });
      });
    }
    ...
    if (filterBar) {
      filterBar.addEventListener("click", function (e) {
        ...
        $all("[data-filter]", filterBar).forEach(function (b) {
          var on = b === btn;
          b.classList.toggle("chip-review", on);
          b.classList.toggle("chip-pending", !on);
    ...
    API.notifications.list().then(function (res) {
      ...
      list.innerHTML =
        '<div class="card border-l-4 border-error"><p class="text-body-md text-on-surface">' +
        T.esc(t("notif.load_fail", "Couldn't load notifications.")) + " " + T.esc(err && err.message || "") +
        '</p></div>';
      if (summary) summary.textContent = t("notif.load_fail", "Couldn't load notifications.");
    });
  }

  /* ============================================================
     GLOBAL: topbar bell — live unread badge + navigation
     Runs on every page that has the bell, whichever shell it uses.
     ============================================================ */
  function initTopbarBell() {
    var bells = $all('[aria-label="Notifications"]');
    if (!bells.length) return;

    var isAdmin = location.pathname.indexOf("/admin") >= 0;
    var target = isAdmin ? "../public/notifications.html" : "notifications.html";
    var onNotifPage = page() === "notifications";

    bells.forEach(function (bell) {
      // Navigation: anchors get a real href, legacy <button> bells get a click.
      if (bell.tagName === "A") {
        if (!bell.getAttribute("href") || bell.getAttribute("href") === "#") bell.setAttribute("href", target);
      } else {
        bell.style.cursor = "pointer";
        bell.addEventListener("click", function () { window.location.href = target; });
      }
    });

    /** Replace the static red dot with a real unread-count badge. */
    function paint(unread) {
      bells.forEach(function (bell) {
        var old = bell.querySelector("[data-bell-badge], .bg-error");
        if (old) old.remove();
        if (!unread || onNotifPage) return;
        var badge = document.createElement("span");
        badge.setAttribute("data-bell-badge", "");
        badge.className =
          "absolute -right-0.5 -top-0.5 grid h-[18px] min-w-[18px] place-items-center rounded-full " +
          "bg-error px-1 text-[10px] font-bold leading-none text-on-error";
        badge.textContent = unread > 9 ? "9+" : String(unread);
        if (getComputedStyle(bell).position === "static") bell.style.position = "relative";
        bell.appendChild(badge);
      });
    }
    window.VarnisUpdateBellBadge = paint;

    API.notifications.unreadCount().then(function (res) {
      paint((res && res.unread) || 0);
    }, function () { /* keep whatever indicator the markup shipped with */ });
  }

  function boot() {
    if (!API || !T) {
      console.warn("VarnisAPI or VarnisTemplates missing");
      return;
    }
    var p = page();
    var map = {
