...
(function () {
  ...
  window.VarnisTemplates = {
    ...
    /** Relative time — "just now", "5m ago", "3h ago", "2d ago", else date. */
    ...
    /** Per-type presentation for the notifications screen. */
    NOTIF_META: {
      alert:     { icon: "campaign",    tone: "bg-error/10 text-error",       href: "monitor.html" },
      report:    { icon: "description", tone: "bg-primary/10 text-primary",   href: "my-reports.html" },
      learning:  { icon: "school",      tone: "bg-secondary/10 text-secondary", href: "learn.html" },
      community: { icon: "groups",      tone: "bg-primary/10 text-primary",   href: "community.html" },
    },

    /** One notification row (public/notifications.html). */
    notificationItem: function (n) {
      var meta = window.VarnisTemplates.NOTIF_META[n.type] ||
        { icon: "notifications", tone: "bg-primary/10 text-primary", href: "index.html" };
      var unread = !n.read;
      return (
        '<article class="card card-hover reveal relative flex cursor-pointer items-start gap-4 transition ' +
          (unread ? "border-l-4 border-primary bg-primary/[0.03]" : "opacity-90") + '"' +
          ' data-notif-id="' + esc(n.id) + '" data-notif-href="' + esc(meta.href) + '"' +
          ' role="button" tabindex="0" aria-label="' + esc(n.title) + (unread ? " (unread)" : "") + '">' +
          '<span class="grid h-11 w-11 shrink-0 place-items-center rounded-full ' + meta.tone + '">' +
            '<span class="material-symbols-outlined text-[22px]">' + meta.icon + '</span>' +
          '</span>' +
          '<div class="min-w-0 flex-1">' +
            '<div class="flex items-baseline justify-between gap-3">' +
              '<h3 class="truncate text-label-lg ' + (unread ? "font-bold text-on-surface" : "font-semibold text-on-surface-variant") + '">' + esc(n.title) + '</h3>' +
              '<span class="shrink-0 text-caption text-outline">' + esc(window.VarnisTemplates.timeAgo(n.createdAt)) + '</span>' +
            '</div>' +
            '<p class="mt-1 text-body-md text-on-surface-variant">' + esc(n.message) + '</p>' +
          '</div>' +
          (unread
            ? '<span class="absolute right-4 top-4 h-2.5 w-2.5 rounded-full bg-primary" title="Unread" aria-hidden="true"></span>'
            : "") +
        '</article>'
      );
    },
