...
| Search | `GET /search?q=` |
| Support | `POST /support/chat` · `GET /support/channels` · `GET/POST /support/tickets` · `GET/PATCH /support/tickets/:id` · `POST /support/tickets/:id/messages` |
| KYC | `GET /kyc/me` · `PUT /kyc/me/draft` · `POST /kyc/me/submit` · `GET /kyc/document-types` · `GET /kyc/submissions` (mod) · `PATCH /kyc/submissions/:id` (mod) |
| Recovery | `GET /recovery/fees` · `GET /recovery/catalog` · `POST /recovery/ownership/verify` · `GET/POST /recovery/cases` · `GET /recovery/cases/:id` · `POST/PATCH /recovery/cases/:id/payment` · `POST /recovery/cases/:id/evidence` · `POST /recovery/cases/:id/evidence-requests` (mod) · `PATCH /recovery/cases/:id/stage` (mod) |
...
Five citizen pages and two management pages were added on top of the original bundle. They follow the same rules as everything else: no `fetch()` in a page, all state through `VarnisAPI`, all copy translatable.

| Page | Purpose |
| --- | --- |
| `public/kyc.html` | Identity verification wizard + all six states |
| `public/recovery.html` | Recovery Center — case list, fees, KYC gate |
| `public/recovery-new.html` | Six-step recovery request (ownership → asset → document → fee → payment → review) |
| `public/recovery-detail.html` | Tracking timeline, payment status, additional-evidence upload |
| `public/support-tickets.html` | Ticket list, composer and conversation thread |
| `admin/kyc.html` | KYC review queue with approve/reject + reason |
| `admin/recovery.html` | Payment confirmation, stage advancement, evidence requests |

`assets/js/features.js` holds the shared vocabulary for these screens (status maps, stage ordering, FCFA formatting, upload guards, the KYC gate). Load it after `api.js`.

### KYC gating

`GET /kyc/me` returns `{ status, personal, document, selfie, submittedAt, expiresAt, rejectionReason, history }` where `status` is one of
`draft → submitted → under_review → approved → rejected → expired`.

Approval is what unlocks incident reporting, the Learning Academy, certification and device recovery. The frontend enforces this with `VarnisFeatures.gateOnKyc()`, which swaps a page's body for a "verify first" panel — **a UX gate, not a security boundary**. Mirror it server-side on every gated endpoint.

Document images travel as base64 data URLs in the same envelope `/auth/register` already uses for `idCard`: `{ fileName, mimeType, size, data }`. The client caps uploads at 5 MB and accepts JPG, PNG, WebP and PDF; enforce both server-side.

### Ownership verification (two-of-three)

`POST /recovery/ownership/verify` takes `{ fullName, phone, password, onBehalf }` and returns `{ verified, matched, matchCount, required: 2 }`.

Any **two of the three** fields must match the owner's account. This is deliberate: a stolen phone usually takes the SIM and the email with it, so demanding all three would lock out the real owner at the worst moment. Two is enough to keep impersonation hard while letting a trusted relative or colleague file on the owner's behalf.

Two implementation rules for the backend:
- Do the comparison **server-side only**. Never ship the owner's stored values to the client.
- Return the match *count*, never *which* field failed — otherwise the endpoint becomes an oracle for probing whether a name or number has an account.

### Recovery workflow

A case moves through nine tracking stages, in this order:

```
ownership_verified → payment_submitted → payment_approved → sent_to_authorities
→ under_investigation → [evidence_requested] → device_located → recovered → closed
```

`evidence_requested` is optional and only rendered when it actually occurred. Payments carry their own `pending | approved | rejected` status. Tracking fees come from `GET /recovery/fees` rather than being hardcoded, so they can change without a redeploy.

When investigators need more documents, `POST /recovery/cases/:id/evidence-requests` attaches a request to the **existing** case — the citizen uploads onto that case rather than opening a new one, which is the whole point of the feature.

### Google sign-in

`login.html` hands off through `VarnisAPI.auth.google()`, which redirects to `GOOGLE_OAUTH_START_PATH` in live mode and emulates a round-trip in mock mode. On return, a brand-new account is routed to `kyc.html` before anything else; an existing account resumes wherever `auth-guard.js` bounced it from. Profile → Security also exposes linking and unlinking Google on an existing account.
...
│   ├── js/              # api.js, config.js, features.js, i18n.js, validation.js, storage.js
│   ├── img/             # Logos / PWA icons
...
VARNIS v2.2.0 (frontend-only) — adds Google sign-in wiring, KYC, device recovery and support tickets
